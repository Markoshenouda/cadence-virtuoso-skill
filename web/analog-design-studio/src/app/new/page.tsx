...
export default function StudioPage() {
  ...
  const [circuitId, setCircuitId] = useState(initialCircuit?.id ?? '');
  ...
  const [specs, setSpecs] = useState<DesignConfig['specs']>(() => defaultSpecsFor(initialCircuit?.id ?? ''));
  const [devices, setDevices] = useState<DeviceDraft[]>(() => defaultDevices(initialCircuit?.id ?? '', initialTopology?.id ?? ''));
