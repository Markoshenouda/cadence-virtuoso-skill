...
export const generatorContracts: Record<string, GeneratorContract> = {
  '5t-ota': contract(
    ...
    'T5TW_Place',
    [
      { device: 'M1', type: 'NMOS' }, { device: 'M2', type: 'NMOS' },
      { device: 'M3', type: 'PMOS' }, { device: 'M4', type: 'PMOS' },
      { device: 'M5', type: 'NMOS' },
    ],
  ),
  'telescopic-ota': contract(
    ...
    [
      { device: 'M1', type: 'NMOS' }, { device: 'M2', type: 'NMOS' },
      { device: 'M3', type: 'NMOS' }, { device: 'M4', type: 'NMOS' },
      { device: 'M5', type: 'PMOS' }, { device: 'M6', type: 'PMOS' },
      { device: 'M7', type: 'PMOS' }, { device: 'M8', type: 'PMOS' },
      { device: 'M9', type: 'NMOS' },
    ],
  ),
  'folded-cascode-ota': contract(
    ...
    [
      ...
      { device: 'M3', type: 'PMOS' }, { device: 'M4', type: 'PMOS' },
      { device: 'M5', type: 'PMOS' }, { device: 'M6', type: 'PMOS' },
      { device: 'M7', type: 'NMOS' }, { device: 'M8', type: 'NMOS' },
      { device: 'M9', type: 'NMOS' }, { device: 'M10', type: 'NMOS' },
...
export function getGeneratorContract(topologyId: string, technologyId: string): GeneratorContract {
  ...
  if (result.technologyId !== technologyId) {
...
export function validateContractConfig(config: DesignConfig, contract: GeneratorContract): void {
  ...
  for (const device of config.devices) {
    ...
    if (expected.get(device.device) !== device.type) throw new Error(`${device.device} must be ${expected.get(device.device)}.`);
    if (!device.totalW.trim() || !device.L.trim()) throw new Error(`${device.device} requires TotalW and L.`);
...
export function deriveMosState(totalW: string, nf: number, m: number) {
  ...
  return { totalW, nf, m, wPerFingerExpression: `(${totalW})/${nf}`, totalM: nf * m };
}
