...
const base = (topologyId: DesignConfig['topologyId'], devices: DesignConfig['devices']): DesignConfig => ({
  circuitId: 'ota', topologyId, technologyId: 'tsmcN65', vdd: 1.2, temperature: 27, corner: 'TT',
  specs: defaultSpecs, sizingMethod: 'manual', devices,
});

const devices = (entries: Array<[string, 'NMOS' | 'PMOS', string, string, number, number]>): DesignConfig['devices'] =>
  entries.map(([device, type, totalW, L, NF, M]) => ({ device, type, totalW, L, NF, M }));

const d5t = devices([
  ['M1','NMOS','10u','300n',2,2], ['M2','NMOS','12u','320n',3,1], ['M3','PMOS','8u','600n',2,1], ['M4','PMOS','10u','600n',5,2], ['M5','NMOS','14u','700n',7,1],
]);
const dTel = devices([
  ['M1','NMOS','11u','900n',2,1], ['M2','NMOS','12u','900n',3,2], ['M3','NMOS','13u','900n',4,1], ['M4','NMOS','14u','900n',2,3],
  ['M5','PMOS','15u','1u',3,1], ['M6','PMOS','16u','1u',4,2], ['M7','PMOS','17u','1u',5,1], ['M8','PMOS','18u','1u',6,2], ['M9','NMOS','19u','1u',7,1],
]);
const dFolded = devices([
  ['M1','NMOS','21u','500n',2,1], ['M2','NMOS','22u','500n',3,2], ['M3','PMOS','23u','900n',2,1], ['M4','PMOS','24u','900n',3,1],
  ['M5','PMOS','25u','1u',4,2], ['M6','PMOS','26u','1u',5,1], ['M7','NMOS','27u','1u',2,3], ['M8','NMOS','28u','1u',3,1],
  ['M9','NMOS','29u','1u',4,2], ['M10','NMOS','30u','1u',5,1], ['M11','NMOS','31u','1u',6,2],
]);
...
describe('Phase 3 generator contracts', () => {
  ...
  it('rejects wrong device count/type before touching source', () => {
    ...
    expect(() => validateContractConfig(base('folded-cascode-ota', dFolded.map((d, i) => i === 2 ? { ...d, type: 'NMOS' } : d)), contract)).toThrow(/M3 must be PMOS/);
  });
