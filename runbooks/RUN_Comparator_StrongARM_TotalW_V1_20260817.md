# Run - StrongARM Comparator TotalW V1

## Generator

```text
canonical/comparator/Comparator_StrongARM_TotalW_V1_20260817.il
```
...
```cmd
scp "C:\Users\<user>\Desktop\Comparator_StrongARM_TotalW_V1_20260817.il" cadence@192.168.75.217:/home/cadence/
```

## Load in CIW

```skill
load("/home/cadence/Comparator_StrongARM_TotalW_V1_20260817.il")
```

## Run

```skill
CreateComparator_StrongARM_TotalW_V1_20260817()
```
...
```text
strongarm-comparator  (circuit: comparator)
```
...
```text
M5 clocked tail; M1/M2 input pair; M6/M7 precharge PMOS; M3/M4 cross-coupled latch; outputs ND1/ND2
```
...
```textV_VDD = 1.50 V   V_CLK = 0.75 V (static stand-in)   V_VINP/V_VINN = 0.75 V   V_VSS = 0.00 V
```
...
```text
ADS_BRIDGE_START
SAC: <device> TotalW=... W/finger=... L=... NF=... M=... totalM=...
SAC: STUB <device>.<terminal> -> <net>
SAC GENERATOR COMPLETED
...
SCHEMATIC_ONLY (DYNAMIC): dynamic clocked circuit; static DC simulation is not meaningful. A clocked transient profile is future work.
