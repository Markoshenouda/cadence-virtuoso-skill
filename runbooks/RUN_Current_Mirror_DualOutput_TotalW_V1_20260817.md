# Run - Dual-Output NMOS Current Mirror TotalW V1

## Generator

```text
canonical/current-mirror/Current_Mirror_DualOutput_NMOS_TotalW_V1_20260817.il
```
...
```cmd
scp "C:\Users\<user>\Desktop\Current_Mirror_DualOutput_NMOS_TotalW_V1_20260817.il" cadence@192.168.75.217:/home/cadence/
```

## Load in CIW

```skill
load("/home/cadence/Current_Mirror_DualOutput_NMOS_TotalW_V1_20260817.il")
```

## Run

```skill
CreateCurrentMirror_DualOutput_NMOS_TotalW_V1_20260817()
```
...
```text
dual-output-current-mirror  (circuit: current-mirror)
```
...
```text
M1 diode reference (G=IREF D=IREF S=VSS B=VSS)
M2 output 1 (G=IREF D=IOUT1 S=VSS B=VSS)
M3 output 2 (G=IREF D=IOUT2 S=VSS B=VSS)
```
...
```textV_IREF = 0.75 V   V_VSS = 0.00 V
```
...
```text
ADS_BRIDGE_START
DOM: <device> TotalW=... W/finger=... L=... NF=... M=... totalM=...
DOM: STUB <device>.<terminal> -> <net>
DOM GENERATOR COMPLETED
...
SIMULATION_READY via the dc-mirror profile (ref=M1, out=M2).
