# Run - Wide-Swing Cascode Current Mirror TotalW V1

## Generator

```text
canonical/current-mirror/Current_Mirror_WideSwing_NMOS_TotalW_V1_20260817.il
```
...
```cmd
scp "C:\Users\<user>\Desktop\Current_Mirror_WideSwing_NMOS_TotalW_V1_20260817.il" cadence@192.168.75.217:/home/cadence/
```

## Load in CIW

```skill
load("/home/cadence/Current_Mirror_WideSwing_NMOS_TotalW_V1_20260817.il")
```

## Run

```skill
CreateCurrentMirror_WideSwing_NMOS_TotalW_V1_20260817()
```
...
```text
wide-swing-cascode-mirror  (circuit: current-mirror)
```
...
```text
M1 ref bottom (G=NB D=NB2 S=VSS B=VSS)
M2 out bottom (G=NB D=NB2R S=VSS B=VSS)
M3 ref cascode (G=NBIAS D=IREF S=NB2 B=VSS)
M4 out cascode (G=NBIAS D=IOUT S=NB2R B=VSS)
M5 bias diode (G=D=NB S=VSS B=VSS)
M6 bias cascode diode (G=D=NBIAS S=NB D=IREF B=VSS)
```
...
```textV_IREF = 1.20 V   V_VSS = 0.00 V   (NB/NBIAS self-biased by the M5/M6 stack)
```
...
```text
ADS_BRIDGE_START
WSC: <device> TotalW=... W/finger=... L=... NF=... M=... totalM=...
WSC: STUB <device>.<terminal> -> <net>
WSC GENERATOR COMPLETED
...
SIMULATION_READY via the dc-mirror profile (ref=M3, out=M4).
