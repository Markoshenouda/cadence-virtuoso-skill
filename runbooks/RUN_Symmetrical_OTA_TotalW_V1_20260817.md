# Run - Symmetrical OTA TotalW V1

## Generator

```text
canonical/ota/Symmetrical_OTA_TotalW_V1_20260817.il
```
...
```cmd
scp "C:\Users\<user>\Desktop\Symmetrical_OTA_TotalW_V1_20260817.il" cadence@192.168.75.217:/home/cadence/
```

## Load in CIW

```skill
load("/home/cadence/Symmetrical_OTA_TotalW_V1_20260817.il")
```

## Run

```skill
CreateSymmetrical_OTA_TotalW_V1_20260817()
```
...
```text
M1 pair (G=VIP D=NA S=TAIL B=VSS)
M2 pair (G=VIN D=NB S=TAIL B=VSS)
M3 diode mirror L (G=D=NA S=VSS B=VSS)
M4 mirror L out (G=NA D=VOUT S=VSS B=VSS)
M5 diode mirror R (G=D=NB S=VSS B=VSS)
M6 mirror R out (G=NB D=VOUT S=VSS B=VSS)
M7 tail (G=VBN_TAIL D=TAIL S=VSS B=VSS)
M8 PMOS load (G=VBP D=VOUT S=VDD B=VDD)
```
...
```text
ADS_BRIDGE_START
SYM: <device> TotalW=... W/finger=... L=... NF=... M=... totalM=...
SYM: STUB <device>.<terminal> -> <net>
SYM GENERATOR COMPLETED
...
SIMULATION_READY via the ota-ac-tran profile.
