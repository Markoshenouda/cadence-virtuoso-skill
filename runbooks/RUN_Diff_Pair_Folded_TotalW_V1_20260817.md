# Run - Folded Differential Pair TotalW V1

## Generator

```text
canonical/differential-pair/Diff_Pair_Folded_TotalW_V1_20260817.il
```
...
```cmd
scp "C:\Users\<user>\Desktop\Diff_Pair_Folded_TotalW_V1_20260817.il" cadence@192.168.75.217:/home/cadence/
```

## Load in CIW

```skill
load("/home/cadence/Diff_Pair_Folded_TotalW_V1_20260817.il")
```

## Run

```skill
CreateDiffPair_Folded_TotalW_V1_20260817()
```
...
```text
folded-diff-pair  (circuit: differential-pair)
```
...
```text
M1 pair (G=VIP D=NFOLD1 S=TAIL B=VSS)
M2 pair (G=VIN D=NFOLD2 S=TAIL B=VSS)
M3 tail (G=VBN_TAIL D=TAIL S=VSS B=VSS)
M4 fold source (G=VBP D=NFOLD1 S=VDD B=VDD)
M5 fold source (G=VBP D=NFOLD2 S=VDD B=VDD)
```
...
```text
ADS_BRIDGE_START
FDP: <device> TotalW=... W/finger=... L=... NF=... M=... totalM=...
FDP: STUB <device>.<terminal> -> <net>
FDP GENERATOR COMPLETED
...
SIMULATION_READY via the dc-diffpair profile (tail=M3, inP=M1, inN=M2).
