# Run - Cascode NMOS Current Source TotalW V1

## Generator

```text
canonical/current-mirror/CurrentSource_Cascode_NMOS_TotalW_V1_20260817.il
```
...
```cmd
scp "C:\Users\<user>\Desktop\CurrentSource_Cascode_NMOS_TotalW_V1_20260817.il" cadence@192.168.75.217:/home/cadence/
```

## Load in CIW

```skill
load("/home/cadence/CurrentSource_Cascode_NMOS_TotalW_V1_20260817.il")
```

## Run

```skill
CreateCurrentSource_Cascode_NMOS_TotalW_V1_20260817()
```
...
```text
nmos-cascode-current-source  (circuit: current-mirror)
```
...
```text
M1 bottom (G=VBN1 D=NC S=VSS B=VSS)
M2 cascode (G=VBN2 D=IOUT S=NC B=VSS)
```
...
```textV_VBN1 = 0.60 V   V_VBN2 = 1.05 V   V_VSS = 0.00 V
```
...
```text
ADS_BRIDGE_START
NCS: <device> TotalW=... W/finger=... L=... NF=... M=... totalM=...
NCS: STUB <device>.<terminal> -> <net>
NCS GENERATOR COMPLETED
...
SIMULATION_READY via the dc-mirror profile (ref/out device roles map to M2 with a series path; treat measurements as indicative).
