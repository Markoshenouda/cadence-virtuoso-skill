# Run - Cascode Amplifier TotalW V1

## Generator

```text
canonical/amplifier/CascodeAmp_NMOS_TotalW_V1_20260817.il
```
...
```cmd
scp "C:\Users\<user>\Desktop\CascodeAmp_NMOS_TotalW_V1_20260817.il" cadence@192.168.75.217:/home/cadence/
```

## Load in CIW

```skill
load("/home/cadence/CascodeAmp_NMOS_TotalW_V1_20260817.il")
```

## Run

```skill
CreateCascodeAmp_NMOS_TotalW_V1_20260817()
```
...
```text
M1  NMOS input   (G=VIN D=NCAS S=VSS B=VSS)
M2  NMOS cascode (G=VBN_CAS D=VOUT S=NCAS B=VSS)
M3  PMOS load    (G=VBP D=VOUT S=VDD B=VDD)
```
...
```text
V_VIN = 0.75 V   V_VBN_CAS = 1.05 V   V_VBP = 1.25 V
V_VDD = 2.00 V   V_VSS = 0.00 V (explicit source)
```
