# Run - PMOS Current Mirror TotalW V1

## Generator

```text
canonical/current-mirror/Current_Mirror_PMOS_TotalW_V1_20260817.il
```
...
```cmd
scp "C:\Users\<user>\Desktop\Current_Mirror_PMOS_TotalW_V1_20260817.il" cadence@192.168.75.217:/home/cadence/
```

## Load in CIW

```skill
load("/home/cadence/Current_Mirror_PMOS_TotalW_V1_20260817.il")
```

## Run

```skill
CreateCurrentMirror_PMOS_TotalW_V1_20260817()
```
...
PMOS = tsmcN65/pch with orientation selected from actual S/D geometry (R0 first, MX fallback).

## Topology

```text
M1  PMOS diode reference (G=IREF D=IREF S=VDD B=VDD)
M2  PMOS output          (G=IREF D=IOUT S=VDD B=VDD)
```

## Initial sizing

```text
M1/M2  TotalW=8u  L=480n NF=1 M=1 (1:1 default)
```
...
```text
V_VDD  = 1.50 V
V_IREF = 0.80 V (reference drive, PLUS=IREF MINUS=VSS)
