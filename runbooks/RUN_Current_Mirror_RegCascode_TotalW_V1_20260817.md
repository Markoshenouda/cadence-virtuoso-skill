# Run - Regulated Cascode Current Mirror TotalW V1

## Generator

```text
canonical/current-mirror/Current_Mirror_RegCascode_NMOS_TotalW_V1_20260817.il
```
...
```cmd
scp "C:\Users\<user>\Desktop\Current_Mirror_RegCascode_NMOS_TotalW_V1_20260817.il" cadence@192.168.75.217:/home/cadence/
```

## Load in CIW

```skill
load("/home/cadence/Current_Mirror_RegCascode_NMOS_TotalW_V1_20260817.il")
```

## Run

```skill
CreateCurrentMirror_RegCascode_NMOS_TotalW_V1_20260817()
```
...
```text
regulated-cascode-mirror  (circuit: current-mirror)
```
...
```textV_IREF = 1.20 V   V_VSS = 0.00 V (explicit source)
```
...
```text
ADS_BRIDGE_START
CRM: <device> TotalW=... W/finger=... L=... NF=... M=... totalM=...
CRM: STUB <device>.<terminal> -> <net>
CRM GENERATOR COMPLETED
...
Simulation-ready via the dc-mirror profile (ref=M3, out=M4; the M5 regulation loop is part of the DC solution).
